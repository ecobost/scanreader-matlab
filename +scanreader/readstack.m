...
% Expand wildcards
filenames = scanreader.utils.expandwildcard(pathnames);
...
tiffInfo = scanreader.tiffutils.gettiffinfo(tiffFile);
version = scanreader.tiffutils.getscanimageversion(tiffInfo);

% Select the appropiate scan object
switch version
    case '5.1'
        stack = scanreader.stacks.Stack5Point1();
    case '5.2'
        stack = scanreader.stacks.Stack5Point2();
    case '2016b'
        if scanreader.tiffutils.isscanmultiROI(tiffInfo)
            stack = scanreader.stacks.StackMultiROI(joinContiguous);
        else
            stack = scanreader.stacks.Stack2016b();
        end
    otherwise
